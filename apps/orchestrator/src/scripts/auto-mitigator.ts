...
export async function applyAutoMitigationLocally(branch: string, recommendedAction: string) {
  try {
    const workspaceRoot = path.resolve(__dirname, '../../../../');
    logger.info(`Applying auto mitigation in ${workspaceRoot} for branch ${branch}`);

    logger.info('Auto-mitigator: Attempting to digest and apply mitigation instructions', {
      recommendedAction,
    });
...
`;
    ...
    if (commandToRun && !commandToRun.includes('```') && !commandToRun.includes('sed')) {
      ...
      if (
        recommendedAction.toLowerCase().includes('whitespace') ||
        recommendedAction.toLowerCase().includes('spac')
      ) {
        logger.info('Whitespace fix fallback triggered.');
        execSync(
          `node -e "const fs = require('fs'); fs.writeFileSync('packages/shared-types/src/dummy.ts', fs.readFileSync('packages/shared-types/src/dummy.ts', 'utf8').trim() + '\\n')"`,
          { cwd: workspaceRoot },
        );
      }
    }
    ...
    execSync('git commit -m "chore: auto-mitigation applied by Agentic CICD"', {
      cwd: workspaceRoot,
      stdio: 'inherit',
    });
