...
export function AgentNode({ data }: NodeProps) {
  ...
  return (
    <div
      className={`relative px-4 py-3 rounded-lg border transition-all duration-500 backdrop-blur-md min-w-[200px] ${STATUS_RING[status]} ${STATUS_BG[status]} group hover:border-white/20`}
    >
      <Handle
        type="target"
        position={Position.Left}
        className="w-1 h-3 rounded-full bg-white/20 border-none -ml-1 transition-opacity opacity-0 group-hover:opacity-100"
      />
      <Handle
        type="source"
        position={Position.Right}
        className="w-1 h-3 rounded-full bg-white/20 border-none -mr-1 transition-opacity opacity-0 group-hover:opacity-100"
      />
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col flex-1">
          ...
          <span
            className={`text-[12px] font-mono tracking-tight capitalize mt-0.5 ${STATUS_TEXT[status]}`}
          >
            {status.replace('_', ' ')}
            {pulse && (
              <motion.span
      ...
      <AnimatePresence>
        ...
        {rebuttalPosition && (
          ...
          >
            <span
              className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest rounded-full shadow-lg ${rebuttalPosition === 'DEFEND' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : rebuttalPosition === 'COMPROMISE' ? 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30' : 'bg-red-500/20 text-red-300 border border-red-500/30'}`}
            >
              {rebuttalPosition}
            </span>
          </motion.div>
